...
if (arrayNewQuestions == null) {
    ...
    arrayQuestions2 = arrayQuestions
} else {
    arrayQuestions2 = arrayQuestions.concat(arrayNewQuestions)
}
...
const filterTopic = (categoryy) => {
    questionsZone.innerHTML = ""
    if (categoryy != "...") {
        arrayQuestions2.map((question, index) => {
            if (question.category == categoryy) {
                questionsZone.innerHTML += `
    <div class="DivChieldQ" id="${question.id}" onclick="answerQuestion('${question.id}')">
        <div class="numberQ">
                                ${index + 1}
...
function answerQuestion(id) {
  ...
  <option value="JS">JS</option>
        ...
        form.addEventListener("submit", (e) => {
            ...
            arrayQuestions2.push(newQuestion)
            arrayNewQuestions.push(newQuestion)
            localStorage.setItem("newQuestions", JSON.stringify(arrayNewQuestions))
            innerQuestions(newQuestion, "A")
        });
    }
}
